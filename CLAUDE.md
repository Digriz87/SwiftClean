...
SwiftClean is a Windows desktop PC-cleaning utility built on **WPF**, targeting **.NET 8** (`net8.0-windows`). The UI is a faithful port of the imported design `SwiftClean.dc.html` (claude.ai/design). All eight pages exist (Dashboard, Cleaning/Очистка, Registry/Реестр, Startup/Автозагрузка, Apps/Приложения, Disk/Диск, Scheduler/Планировщик, Settings/Настройки). **The UI language is Russian.** Scanning is wired to the real `ScannerService` (junk sizes + clean list are real); Registry/Startup/Apps/Disk data is still mock placeholder.
...
There is no test project, linter, or test framework configured yet. If adding tests, create a separate test project and reference `SwiftClean.csproj`; run a single test with `dotnet test --filter "FullyQualifiedName~<TestName>"`.
...
- **Scheduler is real:** "Save schedule" (`SaveScheduleCommand`) calls `SchedulerService` which creates/removes a Windows Task Scheduler task ("SwiftClean Auto-Clean") via the `schtasks` CLI (per-user, no admin) at 03:00 on the chosen frequency. The task runs `SwiftClean.exe --autoclean`, which `App.OnStartup` detects and handles **headlessly** (no window): scans and recycles every safe category, then `Shutdown()`. So `App.xaml` has **no `StartupUri`** — `OnStartup` creates the `MainWindow` for the normal path and branches on `--autoclean`.
- **Switching:** `Loc.SetLanguage` raises the indexer change (refreshes every XAML binding) and fires `LanguageChanged`. `MainViewModel.OnLanguageChanged` then `OnPropertyChanged("")` (refresh all computed VM strings), `NavigationView.Refresh()` (re-group the localized sidebar sections), and rebuilds the scanned lists. Data models that carry localized text (`NavigationItem`, `StartupApp`, `DiskFolder`, `ScanStage`, `CleanItem`, `RegistryIssue`) resolve via `Loc` keys; the long-lived ones subscribe to `LanguageChanged`, the per-scan ones (`CleanItem`/`RegistryIssue`) are rebuilt.
